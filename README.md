...
We study quantum lattices with a localized energy defect.
Mutual information between neighboring sites behaves like a conductance, as in a resistor network.
From these conductances a spatial geometry and causal propagation emerge with the energy defect representing mass.
Quantum corrections to the exterior geometry are order *&xi;*<sup>2</sup>/*&ell;*<sup>2</sup> (squared ratio of correlation length to curvature scale).
Imposing spherical symmetry and self-consistency, a two-state (background-subtracted) closure eliminates Yukawa screening and gives an unscreened far-field 1/*r* exterior.
The gravitational potential that solves the closure equation matches a Schwarzschild exterior at all post-Newtonian orders within the closure model.
Because conductances scale as the squared lapse (*N*<sup>2</sup>), the exterior closure equation reduces to Laplace's equation.
We refer to the resulting object as an *entanglement star*, supported by mutual information conductances that play the role of an effective degeneracy pressure.
The lapse remains strictly positive because conductances are suppressed before the lapse reaches zero, preventing a classical singularity in favor of a conical floor.
Tracking mode-by-mode evaporation in the unitary Gaussian state, the entanglement entropy of emitted radiation follows a Page-like curve.
We also obtain the object's surface gravity, Hawking temperature, and first law.
Fully self-consistent numerical calculations validate the theoretical derivations for free fermions, the Ising model, and XXZ chains in spherical symmetry, and for free fermions on a 3D cubic lattice.
